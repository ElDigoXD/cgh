...
__global__ void test_kernel(const GPUScene &scene, unsigned char *out_pixels, const PointCloudPoint<> *point_cloud, const unsigned int pc_size) {
    const uint x = threadIdx.x + blockIdx.x * blockDim.x;
    const uint y = threadIdx.y + blockIdx.y * blockDim.y;
    if ((x >= IMAGE_WIDTH) || (y >= IMAGE_HEIGHT)) return;
    const uint pixel_index = y * IMAGE_WIDTH + x;
...
__host__ void use_cuda_occ(const Scene &scene, unsigned char pixels[], const PointCloud &point_cloud) {
    ...
    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, test_kernel);
    ...
    test_kernel<<<grid, block>>>(gpu_scene, out_pixels_buff, pc, point_cloud.size());
    CU(cudaGetLastError());
    CU(cudaDeviceSynchronize());
    std::copy_n(out_pixels_buff, num_pixels * 4, pixels);
